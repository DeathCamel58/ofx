module OFX
  ...
  class SignOnResponse < Foundation
    ...
    def self.from_ofx_102(node)
      ...
      response.status = OFX::Status.from_ofx_102(status_node) if status_node

      fi_node = node.search('fi')
      response.financial_institution = OFX::FinancialInstitution.from_ofx_102(fi_node) if fi_node
